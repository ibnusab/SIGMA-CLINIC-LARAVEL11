<?php $__env->startSection('title', 'Input Pemeriksaan Medis Pasien'); ?>
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-sm">
        <div class="flex items-center justify-between pb-6 mb-6 border-b border-slate-100">
            ...
            <a href="<?php echo e(route('medical-records.index')); ?>" class="px-3.5 py-1.5 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-xl text-xs font-bold transition-all">
                &larr; Batal
            </a>
        </div>

        <form action="<?php echo e(route('medical-records.store')); ?>" method="POST" class="space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="bg-sky-50/60 p-4 rounded-2xl border border-sky-100">
                ...
                <select name="registration_id" required class="w-full px-4 py-2.5 bg-white border border-sky-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500">
                    ...
                    <option value="<?php echo e($reg->id); ?>" <?php echo e((old('registration_id') == $reg->id || $selectedRegistrationId == $reg->id) ? 'selected' : ''); ?>>
                        Antrian #<?php echo e(str_pad($reg->queue_number, 2, '0', STR_PAD_LEFT)); ?> - [<?php echo e($reg->patient->mr_number ?? '-'); ?>] <?php echo e($reg->patient->name ?? '-'); ?> (Poli: <?php echo e($reg->clinic->name ?? '-'); ?>)
                    </option>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <input type="text" name="blood_pressure" value="<?php echo e(old('blood_pressure', '120/80')); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500" placeholder="120/80">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Suhu Tubuh (&deg;C)</label>
                        <input type="number" step="0.1" name="temperature" value="<?php echo e(old('temperature', 36.5)); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Tinggi Badan (cm)</label>
                        <input type="number" name="height" value="<?php echo e(old('height', 165)); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500">
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-600 mb-1">Berat Badan (kg)</label>
                        <input type="number" step="0.5" name="weight" value="<?php echo e(old('weight', 60)); ?>" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500">
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <textarea name="complaints" rows="3" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500" placeholder="Keluhan utama dan riwayat penyakit saat ini..."><?php echo e(old('complaints')); ?></textarea>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Pemeriksaan Fisik / Objektif</label>
                    <textarea name="physical_exam" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500" placeholder="Kondisi umum, mata, tenggorokan, thorax, abdomen..."><?php echo e(old('physical_exam')); ?></textarea>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="diagnosis" value="<?php echo e(old('diagnosis')); ?>" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500" placeholder="Contoh: Pharyngitis Akut (ICD-10 J02) / Hipertensi Primer">
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-40 overflow-y-auto p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs">
                    <?php $__currentLoopData = $treatments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <label class="flex items-center space-x-2 p-2 bg-white rounded-xl border border-slate-200 cursor-pointer hover:border-sky-400">
                        <input type="checkbox" name="treatment_ids[]" value="<?php echo e($t->id); ?>" class="rounded text-sky-600 focus:ring-sky-500">
                        <span class="font-bold text-slate-800"><?php echo e($t->name); ?></span>
                        <span class="ml-auto text-emerald-700 font-extrabold">+Rp <?php echo e(number_format($t->price, 0, ',', '.')); ?></span>
                    </label>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-12 gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-200 text-xs">
                        <div class="col-span-5">
                            ...
                            <select name="medicines[0][id]" class="w-full px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold">
                                ...
                                <option value="<?php echo e($m->id); ?>"><?php echo e($m->name); ?> (Stok: <?php echo e($m->stock); ?> <?php echo e($m->unit); ?>) - Rp <?php echo e(number_format($m->selling_price, 0, ',', '.')); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="doctor_notes" value="<?php echo e(old('doctor_notes')); ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500" placeholder="Istirahat cukup, hindari makanan pedas">
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Tanggal Kontrol Ulang (Opsional)</label>
                    <input type="date" name="next_follow_up" value="<?php echo e(old('next_follow_up')); ?>" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-sky-500">
                </div>
            </div>

            <div class="pt-4 border-t border-slate-100 flex items-center justify-end space-x-3">
                <a href="<?php echo e(route('medical-records.index')); ?>" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition-all">
                    Batal
                </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\chrome download\android studio\laravel\sigma-clinic\resources\views/medical-records/create.blade.php ENDPATH**/ ?>