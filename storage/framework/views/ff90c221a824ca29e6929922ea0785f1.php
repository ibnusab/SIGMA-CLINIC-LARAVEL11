...
<head>
    <meta charset="UTF-8">
    <title>Tiket Antrian #<?php echo e($registration->queue_number); ?> - SIGMA CLINIC</title>
    ...
    <div class="w-[300px] bg-white p-6 rounded-2xl shadow-2xl border border-slate-200 text-slate-800 text-center font-mono">
        ...
        <div class="border-y border-dashed border-slate-300 py-3 my-3 text-[11px] text-left space-y-1 font-sans">
            <div class="flex justify-between">
                ...
                <strong class="font-mono text-slate-800"><?php echo e($registration->registration_number); ?></strong>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-500">No. Rekam Medis:</span>
                <strong class="font-mono text-sky-700"><?php echo e($registration->patient->mr_number ?? '-'); ?></strong>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-500">Nama Pasien:</span>
                <strong class="text-slate-900 truncate max-w-[150px]"><?php echo e($registration->patient->name ?? '-'); ?></strong>
            </div>
            <div class="flex justify-between">
                <span class="text-slate-500">Dokter Spesialis:</span>
                <strong class="text-slate-800 truncate max-w-[150px]"><?php echo e($registration->doctor->name ?? '-'); ?></strong>
            </div>
            <?php if($registration->doctor && $registration->doctor->specialization): ?>
            <div class="flex justify-between">
                <span class="text-slate-500">Keahlian:</span>
                <strong class="text-sky-800 font-semibold truncate max-w-[150px]"><?php echo e($registration->doctor->specialization); ?></strong>
            </div>
            <?php endif; ?>
            <div class="flex justify-between">
                <span class="text-slate-500">Jenis Kunjungan:</span>
                <strong class="uppercase text-emerald-700 font-extrabold"><?php echo e($registration->visit_type_label ?? 'Umum (Mandiri)'); ?></strong>
            </div>
            <?php if($registration->complaints || $registration->complaint): ?>
            <div class="flex justify-between">
                <span class="text-slate-500">Keluhan:</span>
                <strong class="text-slate-800 font-semibold truncate max-w-[150px]"><?php echo e($registration->complaints ?? $registration->complaint); ?></strong>
            </div>
            <?php endif; ?>
            <div class="flex justify-between">
                <span class="text-slate-500">Waktu Daftar:</span>
                <span><?php echo e(\Carbon\Carbon::parse($registration->created_at)->format('H:i')); ?> WIB</span>
            </div>
        </div>
...
<?php /**PATH D:\chrome download\android studio\laravel\sigma-clinic\resources\views/registrations/ticket.blade.php ENDPATH**/ ?>