...
@section('title', 'Supplier Farmasi & Pemasok Obat')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-sm flex items-center justify-between">
        ...
        <a href="{{ route('suppliers.create') }}" class="px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-bold shadow-md shadow-sky-600/20 flex items-center space-x-2 transition-all">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 text-xs font-medium">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="py-3.5 px-4 font-mono font-bold text-sky-600">{{ $s->code }}</td>
                        <td class="py-3.5 px-4 font-bold text-slate-900">{{ $s->name }}</td>
                        <td class="py-3.5 px-4 text-slate-700">
                            <i class="fa-solid fa-phone text-sky-500 mr-1"></i> {{ $s->phone }}
                            <div class="text-[10px] text-slate-400">{{ $s->email ?? '-' }}</div>
                        </td>
                        <td class="py-3.5 px-4 text-slate-600 truncate max-w-xs">{{ $s->address ?? '-' }}</td>
                        <td class="py-3.5 px-4">
                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-800">
                                {{ $s->medicines_count }} Jenis Obat
                            </span>
                        </td>
                        <td class="py-3.5 px-4 text-center">
                            <a href="{{ route('suppliers.edit', $s) }}" class="text-sky-600 hover:text-sky-800 font-bold">
                                Edit Supplier
                            </a>
                        </td>
                    </tr>
