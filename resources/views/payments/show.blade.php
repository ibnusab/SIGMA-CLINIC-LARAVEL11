...
@section('title', 'Rincian Billing & Kasir - ' . $payment->payment_number)
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white p-8 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
        ...
        <div class="flex items-center justify-between pb-6 border-b border-slate-100">
            ...
            <a href="{{ route('payments.index') }}" class="px-3.5 py-1.5 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-xl text-xs font-bold transition-all">
                &larr; Kembali
            </a>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div>Nama Pasien: <strong class="text-slate-900 block font-bold text-sm">{{ $payment->patient->name ?? '-' }}</strong> (RM: {{ $payment->patient->mr_number ?? '-' }})</div>
            <div>Dokter Pemeriksa: <strong class="text-slate-900 block font-bold text-sm">{{ $payment->registration->doctor->name ?? '-' }}</strong></div>
            <div>Status Kunjungan: <strong class="uppercase font-bold text-sky-700">{{ $payment->registration->visit_type ?? 'Umum' }}</strong></div>
            <div>Status Tagihan: 
                <span class="px-2 py-0.5 rounded text-[10px] font-black uppercase {{ in_array(strtolower($payment->status), ['paid', 'lunas']) ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900' }}">
                    {{ $payment->status }}
                </span>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs">
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        <tr>
                            <td class="p-3 font-bold text-slate-800">Biaya Konsultasi Dokter ({{ $payment->registration->doctor->name ?? 'Dokter' }})</td>
                            <td class="p-3 text-right">1 Jasa</td>
                            <td class="p-3 text-right font-bold text-slate-900">Rp {{ number_format($payment->consultation_fee, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold text-slate-800">Total Tindakan Medis</td>
                            <td class="p-3 text-right">-</td>
                            <td class="p-3 text-right font-bold text-slate-900">Rp {{ number_format($payment->treatment_fee, 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td class="p-3 font-bold text-slate-800">Total Pengambilan Obat Apotek</td>
                            <td class="p-3 text-right">-</td>
                            <td class="p-3 text-right font-bold text-slate-900">Rp {{ number_format($payment->medicine_fee, 0, ',', '.') }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr class="bg-slate-900 text-white font-extrabold text-sm">
                            ...
                            <td class="p-4 text-right text-emerald-400">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <!-- Payment Action Box -->
        @if(!in_array(strtolower($payment->status), ['paid', 'lunas']))
        <div class="bg-sky-50 p-6 rounded-3xl border border-sky-100 space-y-4">
            ...
            <form action="{{ route('payments.pay', $payment) }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    <div>
                        ...
                        <input type="number" name="paid_amount" value="{{ $payment->total_amount }}" required min="{{ $payment->total_amount }}" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl font-black text-slate-900 focus:outline-none">
                    </div>
                </div>
        ...
        <div class="p-4 bg-emerald-50 rounded-2xl border border-emerald-200 text-emerald-900 flex items-center justify-between text-xs">
            <div>
                ...
                <span>Metode: <strong class="uppercase">{{ $payment->payment_method }}</strong> | Lunas pada: {{ \Carbon\Carbon::parse($payment->paid_at)->isoFormat('D MMMM YYYY, HH:mm') }} WIB</span>
            </div>

            <a href="{{ route('payments.invoice', $payment) }}" target="_blank" class="px-5 py-2.5 bg-emerald-700 hover:bg-emerald-800 text-white font-bold rounded-xl shadow transition-all flex items-center space-x-2">
